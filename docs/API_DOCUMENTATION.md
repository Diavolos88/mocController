...
### 7. Удаление конфигурации (Web UI)
...
- `steps` (array, необязательное) - список шагов:
...
- Если указан `delayMs` - применяет с задержкой относительно предыдущего шага
- Если указан `scheduledTime` - применяет в указанное время
